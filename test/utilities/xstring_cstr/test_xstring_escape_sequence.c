...
CESTER_TEST(xstring_cstr_escape_sequences, _, {
    ...
    cester_assert_uint_eq(xstring_cstr_escape_sequences(allocator, cstr2, &value, XTD_NULL), XTD_OK);
    cester_assert_str_equal(value, cstr1);
    cester_assert_str_equal(value, "Hello\tWorld");
    cester_assert_uint_ne(xstring_cstr_escape_sequences(allocator, cstr3, &value, XTD_NULL), XTD_OK);
    cester_assert_uint_eq(xstring_cstr_escape_sequences(allocator, cstr3, &value, &err_pos), XTD_PARSING_ERR);
    ...
    cester_assert_uint_eq(xstring_cstr_escape_sequences(allocator, cstr5, &value, &err_pos), XTD_OK);
...
CESTER_TEST(xstring_cstr_unescape_sequences, _, {
    ...
    cester_assert_str_not_equal(cstr2, "Hello\tWorld");
    cester_assert_uint_eq(xstring_cstr_unescape_sequences(allocator, cstr1, &value, XTD_NULL), XTD_OK);
    ...
    cester_assert_str_not_equal(cstr5, cstr4);
    cester_assert_uint_eq(xstring_cstr_unescape_sequences(allocator, cstr4, &value, &err_pos), XTD_OK);
