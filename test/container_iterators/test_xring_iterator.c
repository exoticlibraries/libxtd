...
CESTER_TEST(xring_iterator, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;

    status = xring_new(str)(&fruits, 4);
    cester_assert_uint_eq(status, XTD_OK);
    status = xring_enqueue(str)(fruits, "Apple");
    cester_assert_uint_eq(status, XTD_OK);
    status = xring_enqueue(str)(fruits, "Banana");
    cester_assert_uint_eq(status, XTD_OK);
    status = xring_enqueue(str)(fruits, "Mango");
    cester_assert_uint_eq(status, XTD_OK);
    status = xring_enqueue(str)(fruits, "Pineapple");
    cester_assert_uint_eq(status, XTD_OK);
    
    iterator = XITERATOR_INIT(xring, str, fruits);
...
CESTER_TEST(xring_iterator_reverse, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_foreach_reverse, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_foreach_with_index, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_foreach_reverse_with_index, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_modify_in_loop, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
    str value;
    ...
    iterator = XITERATOR_INIT(xring, str, fruits);
    XFOREACH_INDEX(size_t index, const char *fruit, iterator, {
        if (xring_size(fruits) > 3) {
            status = xring_dequeue_at(str)(fruits, 3, &value);
            cester_assert_uint_eq(status, XTD_OK);
        }
        if (xring_size(fruits) > 2) {
            status = xring_dequeue_at(str)(fruits, 1, &value);
            cester_assert_uint_eq(status, XTD_OK);
        }
...
CESTER_TEST(xring_iterator_advance_by, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_iterator_increment_decrement, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_iterator_segfault, _, {
    xring(str) *fruits;
    enum x_stat status;
    XIterator *iterator;
...
CESTER_TEST(xring_iterator_forward_and_backward, _, {
    xring(str) *fruits;
    enum x_stat status;
    ...
    index = 0;
    forward_iterator = XITERATOR_INIT(xring, str, fruits);
    backward_iterator = XITERATOR_INIT(xring, str, fruits);
...
CESTER_OPTIONS(
    CESTER_TEST_SHOULD_FAIL(xring_iterator_segfault);
